...
tRNA2plot <- left_join(tRNA_tib3,pval %>% filter(Figure=="S11") %>% select(feat_name=type,signif,pval_adj)) %>% filter(n>3)
...
p1 <- ggplot(toplot1)+
	...
	geom_text(aes(x=feat_name,y=4300,label=signif),col="grey20",size=6) +
...
p0 <- p1/p2/p3/p4 & theme(legend.position = "bottom",axis.title.x=element_blank())
p0 + plot_layout(guides = "collect")
ggsave(paste0(path_figures,"FigureS11Aalt.pdf"),h=12,w=12)
...
p0 <- p1/p2/p3/p4 & theme(legend.position = "bottom",axis.title.x=element_blank())
p0 + plot_layout(guides = "collect")
ggsave(paste0(path_figures,"FigureS11Balt.pdf"),h=12,w=12)
...
p0 <- p1/p2/p3/p4 & theme(legend.position = "bottom",axis.title.x=element_blank())
p0 + plot_layout(guides = "collect")
ggsave(paste0(path_figures,"FigureS11Calt.pdf"),h=12,w=12)
