...
f4a <- ggplot(toplot,aes(x=HU,y=mea))+
	...
	geom_point(data=toplot2,aes(x=HU,y=m),col="red",shape=95,alpha=0.6,size=10,show.legend=F)+
	geom_errorbar(data=toplot2,aes(x=HU,y=m,ymin=m-sd,ymax=m+sd),col="red",width=0.2,alpha=0.6)+
	geom_text(data=toplot2,aes(x=HU,y=0,label=n),col="grey40",fontface="italic") +
	...
	theme(axis.ticks.x = element_blank(),legend.position = "none",plot.tag=element_text(face="bold"))+
	xlab("HU (mM)")+
	ylab("Speed (bp/min)")+
	labs(tag="a")
...
f4b <- ggplot(toplot,aes(x=mutant,y=mea))+
	stat_dots(col="grey40",size=1,shape=16,side="both")+
	geom_point(data=toplot2,aes(x=mutant,y=m),col="red",shape=95,alpha=0.6,size=10,show.legend=F)+
	geom_errorbar(data=toplot2,aes(x=mutant,y=m,ymin=m-sd,ymax=m+sd),col="red",width=0.2,alpha=0.6)+
	geom_text(data=toplot2,aes(x=mutant,y=0,label=n),col="grey40",fontface="italic") +
